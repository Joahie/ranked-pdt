  ...
  async execute(interaction) {
    ...
    collector.on('collect', async i => {
      ...
      if(i.customId === confirmId){
        ...
        if(judgeDB.judged == null){
          var judged = 1;
        }else{
          var judged = judgeDB.judged + 1;
        }
    ...
    .setDescription('<@'+interaction.user.id+'> has gained an elo boost for judging a round. They now have ' + eloBoosts + " boosts and have judged "+judged +" rounds this season.")
    .addFields(
      { name: 'Government Team', value: govTeamName, inline: false},
      { name: 'Opposition Team', value: oppTeamName, inline: false},
      { name: 'Resolution', value: resolution, inline: false},		)
    await mongoUsers.updateOne({id: interaction.user.id},{$set:{eloBoosts: eloBoosts, judged:judged}})
