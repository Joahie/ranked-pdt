...
module.exports = {
	...
	data: new SlashCommandBuilder()
		.setName('qualified-leaderboard')
		.setDescription('View the leaderboard that only contains debaters with 3 or more wins')
	...
	async execute(interaction) {
		...
		const count = await mongoUsers.count({ wins: { $gt: 2 } });
		...
		const results = await mongoUsers.find({ wins: { $gt: 2 } }).toArray();
		var eloArray = [];
		var nameArray = [];
		var idArray = [];
	...
	.setColor(0x0099FF)
	.setTitle("Qualified Leaderboard")
