...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    .setName('judging-leaderboard')
    .setDescription('View the judging leaderboard')
  ...
  async execute(interaction) {
    ...
    const count = await mongoUsers.count({ judged: { $gt: 0 } , deleted: {$ne: true}})
    if(count == 0){
      return interaction.reply({content: "Currently no competitors have judged"})
    }
    ...
    const results = await mongoUsers.find({ judged: { $gt: 0 } , deleted: {$ne: true}}).toArray()
    var eloArray = [];
    var nameArray = [];
    var idArray = [];
    for(i = 0; i<results.length; i++){
      eloArray.push(results[i].judged)
