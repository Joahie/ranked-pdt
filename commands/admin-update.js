...
if(dooppBoosts){
    ...
    var oppEloBoost = ""
			...
			var row = new ActionRowBuilder()
			 ...
			 interaction.channel.send({content: "<@"+interaction.user.id+"> please confirm or deny the results of this round. If you don't respond within 15 minutes, the results will be automatically validated."})
			 var filter = i => {
		...
		collector.on('end', async collected => {
			...
			if(collected.size == 0){
				...
				await interaction.channel.send({content: "<@"+interaction.user.id+"> didn't respond within 15 minutes, so round #" + roundID+" (reported by <@" +interaction.user.id+ ">) has been automatically validated."})
					return interaction.channel.send({ embeds: [embed]});
			...
			collector.on('collect', async i => {
				...
				await i.update({components: [greyOut] });
					...
					if(i.customId === confirmId){
						...
						await interaction.followUp({content:"The results of round #"+roundID+" have been confirmed by <@"+interaction.user.id+">"})
						if(R_G > govDB.topElo){
